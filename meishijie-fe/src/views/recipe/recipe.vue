<template>
  <div class="recipe">
    <!-- v-model="activeName" -->
    <!-- 菜谱分类 start -->
    <!-- v-model="activeName" 初始化一级标题的激活样式 this.$route.query[0] this.$route.query===option.type 3-1 二级标题每一项对应 watch:{监听缓存着}  -->
    ...
    <el-container>
      ...
      <el-main class="filter-menus-box">
        ...
        <div style="text-align: right" v-show="!loading">
          <!-- 只有一页时是否隐藏 :hide-on-single-page 	@size-changepageSize 改变时会触发 page-size每页显示条目个数，支持 .sync 修饰符
          @current-change currentPage 改变时会触发  :current-page:sync 当前页数，支持 .sync 修饰符
...
export default {
  ...
  methods: {
    // current-page:sync ="pages.currentPage" 直接异步处理 不用我们自己做请求
    changePage() {
      this.$router.push({
        query: {
          ...this.$route.query,
    ...
    getMenus() {
      ...
      spinner 自定义加载图标类名    spinner:"el-icon-loading"  text	显示在加载图标下方的加载文案
      ...
      */
      ...
      this.list = []; // 每次请求之前都清空 配合loading优化体验
      this.loading = true;
      // 分页 找出加载表单 页码条目个数 总条目数  当前页数 总共的页数是总条数除以每页条目个数
    ...
    tabClickHandle(tab, event) {
      ...
      // 写分类  查询的每一项 里面的夫组件名字  里面的表单类型查询点击切换 路由显示查询和查询的类型
      // const classifyName = this.classifyName;
      // const item = this.classifies.find((item) => item.parent_type === classifyName);
      // console.log(item);
      // if (item) {
      //   this.classifyName = item.parent_name;
      //   this.classifyType = item.list[0].type;
      //   this.$router.push({
      //     query: {
      //       ...this.$route.query,
