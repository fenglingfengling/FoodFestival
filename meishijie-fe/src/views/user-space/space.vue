<template>
  <div class="space">
    ...
    <!-- v-model="activeName" 12 切换tab 标签页 事件名称	说明	回调参数 tab-click	tab 被选中时触-->
...
export default {
  ...
  methods: {
    async getInfo() {
      ...
      //    this.list = data.list;
      // }
      // 闭包应用 当前项 有序化队列方式解决 粉丝 作品 粉丝  works = 作品的数据  fans = 粉丝的数据 所以用闭包 函数的形参和当前数据一一相对应
    ...
    tabClickHandler() {
      ...
      // console.log(this.$route,this.$route.query); //14 查询用户ID query:{userId:"5d613121ac558b4d879824be"} 根据name属性切换路由}}}
    },
  },
};
</script>
